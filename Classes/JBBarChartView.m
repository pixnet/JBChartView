...
- (void)reloadData {
    ...
    dispatch_block_t createDataDictionaries = ^{
        ...
        NSAssert([self.dataSource respondsToSelector:@
                selector(numberOfBarsInBarChartView:)], @"JBBarChartView // datasource must implement - (NSUInteger)numberOfBarsInBarChartView:(JBBarChartView *)barChartView");
        ...
        NSAssert([self.delegate respondsToSelector:@
                selector(barChartView:heightForBarViewAtAtIndex:)], @"JBBarChartView // delegate must implement - (CGFloat)barChartView:(JBBarChartView *)barChartView heightForBarViewAtAtIndex:(NSUInteger)index");
...
- (NSInteger)barViewIndexForPoint:(CGPoint)point {
    NSUInteger index = 0;
    NSUInteger selectedIndex = (NSUInteger) kJBBarChartViewUndefinedBarIndex;

    if (point.x < 0 || point.x > self.bounds.size.width) {
        return selectedIndex;
    }

    CGFloat padding = (CGFloat) ceil(self.barPadding * 0.5);
    for (UIView *barView in self.barViews) {
...
- (UIView *)barViewForForPoint:(CGPoint)point {
    ...
    if (selectedIndex >= 0) {
        return [self.barViews objectAtIndex:(NSUInteger) [self barViewIndexForPoint:point]];
    }
    return barView;
}
...
- (void)touchesBeganOrMovedWithTouches:(NSSet *)touches {
    ...
    if ([self.delegate respondsToSelector:@selector(barChartView:didSelectBarAtIndex:touchPoint:)]) {
        [self.delegate barChartView:self didSelectBarAtIndex:(NSUInteger) [self barViewIndexForPoint:touchPoint] touchPoint:touchPoint];
    }

    if ([self.delegate respondsToSelector:@selector(barChartView:didSelectBarAtIndex:)]) {
        [self.delegate barChartView:self didSelectBarAtIndex:(NSUInteger) [self barViewIndexForPoint:touchPoint]];
    }
}
